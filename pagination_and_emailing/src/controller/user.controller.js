...
router.post('/', async (req,res) => {
    try{
        ...
        sendMail(
            ...
            `Hi ${user.first_name}, Confirm your email address`
        );

        const aMails = await User.find({'role': 'admin'}, {'email': true , "_id": false}).limit(5).lean().exec();

        let mails = [];
        aMails.forEach( ({ email }) => {
            mails.push(email);
        })
        ...
        sendMail(
            'site@a.com',
            mails,
            `${user.first_name} ${user.last_name} has succesfully registered with us`,
